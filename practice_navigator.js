...
export class PracticeNavigator {
  constructor({ getNextItem, onRender, deckId = 'default', persist = true, storage = window.localStorage }) {
    ...
    this.key = `practiceHistory:${this.deckId}`;
    this.history = [];
    this.pos = -1;
    this._load();
  }
  ...
  next(){
    if (this.pos < this.history.length - 1) { this.pos++; this._save(); this._renderCurrent(); return this.current(); }
  ...
  _pushFromSource(){
    const got = this.getNextItem();
    let item = got, meta=null;
  ...
  _load(){ if(!this.persist) return; try{ const raw=this.storage.getItem(this.key); if(!raw) return;
    const s=JSON.parse(raw); if(Array.isArray(s.history)&&typeof s.pos==='number'){ this.history=s.history; this.pos=Math.min(Math.max(0,s.pos), this.history.length-1); } }catch{}
